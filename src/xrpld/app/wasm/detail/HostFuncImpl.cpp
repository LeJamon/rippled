    ...
    Slice const& p2_uncompressed_be64)
{
    ...
    try
    {
        if (p1_uncompressed_be64.size() != G1_LENGTH ||
            p2_uncompressed_be64.size() != G1_LENGTH)
            return Unexpected(HostFunctionError::INVALID_PARAMS);
    ...
    Slice const& scalar_uncompressed_be32)
{
    ...
    {
        if (p1_uncompressed_be64.size() != G1_LENGTH ||
            scalar_uncompressed_be32.size() != SCALAR_LENGTH)
            return Unexpected(HostFunctionError::INVALID_PARAMS);
...
Expected<Bytes, HostFunctionError>
WasmHostFunctionsImpl::bn254NegHelper(Slice const& p1_uncompressed_be64)
{
    ensure_bn254_init();
    try
    {
...
Expected<int32_t, HostFunctionError>
WasmHostFunctionsImpl::bn254PairingHelper(Slice const& pairs)
{
